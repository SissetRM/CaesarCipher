...
class Message(object):
    ...
    def build_shift_dict(self, shift):
        ...
        for letters in range(len(upper)):
            try:
                cipher.__setitem__(upper[letters], upper[letters + shift])
            except IndexError:
                cipher.__setitem__(upper[letters], upper[letters + shift - 26])
        for letters in range(len(lower)):
            try:
                cipher.__setitem__(lower[letters], lower[letters + shift])
            except IndexError:
                cipher.__setitem__(lower[letters], lower[letters + shift - 26])
        return cipher
    ...
    def apply_shift(self, shift):
        ...
        for i in range(len(self.get_message_text())):
            if message[i] in cipher.keys():
                message[i] = cipher.__getitem__(message[i])
        return "".join(message)
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        for i in range(26):
            ...
            for words in shifted_message.split(' '):
                if words in self.valid_words:
                    num_valid_words += 1
            if num_valid_words > tot_valid_words:
                tot_valid_words = num_valid_words
                decrypted_text = shifted_message
                best = i
            num_valid_words = 0
        return best, decrypted_text
...
def decrypt_story():
    ...
    cipher_story = CiphertextMessage(text)
